...
class spectra:
    ...
    def autocorr(self, roi, t_start, t_stop, lstart, lstop):
        ...
        def parab(x, a, b, c): return a*x**2 + b*x + c
    ...
    def CVI_529_line_generator(self, mu_add, kbt, A,sim=False):
        ...
        def doppler_broadening(kbt, a): return a * \
            np.sqrt(2*1.602176487*abs(kbt)/19.9419) / 30000
        s = doppler_broadening(kbt, mu)
        def gauss(lambd, s, A, up, down): return A * \
    ...
    def CVI_fitfunc(self, esti):
        ...
        kbt = esti[1]
        A = esti[2]
        modelled = self.CVI_529_line_generator(mu_add, kbt, A)
    ...
    def CVI_fitfunc_sim(self, esti):
        ...
        A = esti[2]
        modelled = self.CVI_529_line_generator(mu_add, kbt, A)
        C = (modelled - self.simulated)/self.active.error
    ...
    def CVI_fitfunc_plot_sim(self, esti):
        ...
        fs = 15
        plt.figure()
        plt.errorbar(lambd, self.simulated, self.active.error, color="blue")
        plt.plot(lambd, modelled, color="red")
        plt.xlabel("Wavelength [nm]", fontsize=fs)
        plt.ylabel("Intensity [a.u.]", fontsize=fs)
        plt.grid()
...
def wavelength_calib(dataobj, grid, roi, wl):
    ...
    # exchanging coordinates: pixel -> wavelength
    dataobj.del_coordinate("Coord 2")
    return dataobj
...
def slice_by_wl_range(spectra, wstart, wstop, expe_id, roi, timerange):
    spectra_1w = spectra.slice_data(slicing={"ROI": "P0"+str(roi)})
    spectra_1w = spectra_1w.slice_data(slicing={"Wavelength": flap.Intervals(
        wstart, wstop)}, summing={"Wavelength": "Mean"})
    ...
    plt.ylabel("Intensity", fontsize=15)
    plt.title(
        "Time evolution of a pixel at wavelength ["+str(wstart)+", "+str(wstop)+"] nm")
...
def active_passive_with_error(spectra, roi, t_start, t_stop, expe_id, timerange, bg_wls=[0], plots=False):
    ...
    ROI1 = spectra.slice_data(
        slicing={"ROI": "P0"+str(roi), "Time": flap.Intervals(t_start, t_stop)})
    s_on_sliced = ROI1.slice_data(
        slicing={'Time': d_beam_on})  # for error calculation
...
def autocorr(qsi_cxrs, roi, t_start, t_stop, lstart, lstop, expe_id):
    ...
    plt.xlabel(r'$\tau [s]$', fontsize=fs)
    plt.title(
        expe_id+", Auto-correlation, $\lambda = ["+str(lstart)+", "+str(lstop)+"]$ nm")
    plt.grid()
...
def normed_intensity(qsi_cxrs, grid, t_start, t_stop, expe_id, timerange, lstart, lstop, bg_wls):
    ...
    c = d_beam_off.get_coordinate_object("Time")
    c.start = c.start + el
    qsi_cxrs = qsi_cxrs.slice_data(
        slicing={"Time": flap.Intervals(t_start, t_stop)})
    if(bg_wls != [0]):
        ...
        for i in range(qsi_cxrs_bg.data.shape[0]):
            qsi_cxrs.data[:, i, :] = qsi_cxrs.data[:, i, :] - \
                qsi_cxrs_bg.data[i]
    s_on = qsi_cxrs.slice_data(slicing={'Time': d_beam_on}, summing={
                               "Rel. Time in int(Time)": "Mean"})
    s_off = qsi_cxrs.slice_data(slicing={'Time': d_beam_off}, summing={
                                "Rel. Time in int(Time)": "Mean"})
    s_subs = s_on
    s_subs.data = s_on.data-s_off.data

    s_subs = s_subs.slice_data(
        slicing={"Wavelength": flap.Intervals(lstart, lstop)})
    ...
    lambd = s_on.coordinate("Wavelength")[0]
    def gaus(x, A, s, mu, C): return C + A*np.e**(-(((x-mu)**2)/s**2))/s
    ...
    for i in range(4):
        ...
        popton, pcovon = curve_fit(
            gaus, lambd, y, p0=[max(y), 0.1, lambd.mean(), 0])
        # plt.figure()
        # plt.plot(lambd,y,"b+")
        # plt.plot(lambd,gaus(lambd,*popton))
...
def Li_spectrum(B, E, v, n, isotopemass):
    ...
    if(isotopemass == 6):
        # 6Li 3P1/2 excited state energy relative to ground state (E/h) in Hz
        E_D1 = 446.789598e12
        # 6Li 3P3/2 excited state energy relative to ground state (E/h) in Hz
        E_D2 = 446.799650e12
    elif(isotopemass == 7):
        # 7Li 3P1/2 excited state energy relative to ground state (E/h) in Hz
        E_D1 = 446.800132e12
        # 7Li 3P3/2 excited state energy relative to ground state (E/h) in Hz
        E_D2 = 446.810184e12
    else:
    ...
    if (vabs > 0):
        Lambda = np.matrix([[gam, -gam*v[0]/c, -gam*v[1]/c, -gam*v[2]/c],  # Lorentz boost tensor
                            [-gam*v[0]/c, 1+(gam-1)*v[0]**2 / vabs**2, v[0]*v[1]
                             * (gam-1) / vabs**2, v[0]*v[2]*(gam-1) / vabs**2],
                            [-gam*v[1]/c, v[0]*v[1]*(gam-1) / vabs**2, 1+(
                                gam-1)*v[1]**2 / vabs**2, v[1]*v[2]*(gam-1) / vabs**2],
    ...
    F_mf = np.matmul(Lambda, (np.conjugate(
        np.matmul(Lambda, (np.conjugate(F_lab).T))).T))
    B_mf = np.array([F_mf[3, 2], -F_mf[3, 1], F_mf[2, 1]]
                    )        # moving frame B field
    # magnitude of B in moving frame
    B_abs = np.sqrt(sum(B_mf**2))
    if (B_abs > 0):
    ...
    # compute delta m_J
    # the m_J operator is diagonal on our base
    J_z = np.matrix(np.diag([-3/2, -1/2, -1/2, 1/2, 1/2, 3/2]))
    deltam = np.zeros(12)

    for i in range(6):
        deltam[i] = (-1/2) - np.transpose(vecs[:, i]) * \
            (J_z*vecs[:, i])  # decaying to state |g,0,-1>
        deltam[i+6] = (1/2) - np.transpose(vecs[:, i]) * \
            (J_z*vecs[:, i])  # decaying to state |g,0,1>
...
def CVI_529_line_generator(grid, roi, wavelength_setting, lower_wl_lim, upper_wl_lim, mu_add, kbt, A, dslit, zsplit):
    ...
    wl_values = wavelength_grid_generator_op21(
        grid, wavelength_setting, roi)  # loading the wavelength grid
    # slicing the wavelength grid
    ...
    instr = np.load("instr_funcs/"+grid+"_P03_" +
                    str(int(dslit))+"micron_slit.npy").ravel()
    complete_spectrum = np.convolve(doppler_spectrum, instr, mode="same")
...
def Li_line_generator(B, roi, grid, wavelength_setting, lower_wl_lim, upper_wl_lim, mu_add, A):
    ...
    wl_values = flap_w7x_abes.wavelength_grid_generator(
        grid, wavelength_setting, roi)
...
def CVI_fitfunc(esti):
    ...
    modelled = CVI_529_line_generator(
        grid, roi, wavelength_setting, lower_wl_lim, upper_wl_lim, mu_add, kbt, A, dslit, zspl)
...
def Li_fitfunc(esti):
    mu_add = esti[0]
    A = esti[1]
    # maybe we should leave that const
    ...
    modelled = Li_line_generator2(
        B, roi, grid, wavelength_setting, lower_wl_lim, upper_wl_lim, mu_add, A, R, M)
...
def CVI_fitfunc_plot(measured, measured_err, lambd, mu_add, kbt, A, expe_id, grid, ws, roi, tstart,
    ...
    zspl = np.loadtxt("current_zeeman_components.txt")
    modelled = CVI_529_line_generator(
        grid, roi, ws, lvl, uvl, mu_add, kbt, A, dslit, zspl)
    fs = 15
    plt.figure()
...
def Li_Bfit(spectra, mu_add, A, expe_id, grid, ws, roi, tstart, tstop, bg, B, lvl, uvl, tr, R, M):
    ...
    measured = measured.slice_data(
        slicing={"Wavelength": flap.Intervals(lvl, uvl)})
    save_spectral_config([grid, roi, ws, lvl, uvl, R, M])
    np.save("Li_670nm_P0"+str(roi)+"_"+grid+"_measurement", measured.data)
    np.save("Li_670nm_P0"+str(roi)+"_"+grid +
            "_measurement_error", measured.error)
...
def tempfit_error_fit(sol, fmin, met):
    np.savetxt("Terror_fit_parameters.txt",
               np.array([sol[0], sol[1], sol[2], fmin]))
...
def CVI_tempfit(spectra, mu_add, kbt, A, expe_id, grid, ws, roi, tstart, tstop, bg, B, theta, lvl, uvl, tr, dslit):
    ...
    np.save("CVI_529nm_P0"+str(roi)+"_"+grid +
            "_measurement_error", measured.error)
    ...
    if(solution.success == True):
        ...
        plt.title("R = "+str(R_plot)+" m, $\chi^2 = $"+str(round(solution.fun, 6)
                                                           )+", $T_C$ = "+str(round(sol[1], 2))+" $\pm$ 78.65 eV")
        # N = 1000
...
def CVI_line_simulator(measured0, mu_add, kbt, A, expe_id, grid, ws, roi, tstart,
    ...
    if(plots == True):
        wl_values = wavelength_grid_generator_op21(
            grid, ws, roi)  # loading the wavelength grid
    ...
    calculated = CVI_529_line_generator(
        grid, roi, ws, lvl, uvl, mu_add, kbt, A, dslit, zsplit)
    ...
    err = sq(calculated*10, errparam[0], errparam[1])
...
def CVI_line_simulator_me(measured0, mu_add, kbt, A, expe_id, grid, ws, roi, tstart,
    ...
    calculated = CVI_529_line_generator(
        grid, roi, ws, lvl, uvl, mu_add, kbt, A, 100, zc)
    calculated = gridfac*scalef*popt[0]*calculated/max(calculated)
    ...
    for i in range(iter_num):
        ...
        if(plots == True):
            wl_values = wavelength_grid_generator_op21(
                grid, ws, roi)  # loading the wavelength grid
            # slicing the wavelength grid
            wl_grid0 = wl_values[wl_values > lvl]
            lambd = wl_grid0[uvl > wl_grid0]
        ...
        if(plots == True):
            ...
            plt.title("R = "+str(R_plot)+" m, $\chi^2 = $" +
                      str(round(solution.fun, 6))+", $T_{C6+}$ = "+str(round(sol[1], 2)))
...
def spectrum_deconv(spectra, grid, roi, lvl, uvl, n, K):
    ...
    res = spectra.coordinate("Wavelength")[
        0][1]-spectra.coordinate("Wavelength")[0][0]
    ...
    plt.ylabel("$Intensity$", fontsize=15)
    plt.title("20181018_010, channel 43, m/n = " +
