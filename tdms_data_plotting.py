...
def plot_tdms_data(channels=['HV Em Meas Voltage','HV Em Meas Current','HV Ex Meas Voltage','HV Ex Meas Current','TC Top'], search_dir='/data/W7-X/APDCAM/',save_dir='/data/W7-X/Beam/tdms_processed/',shot=None,group_name=None):
    plt.close('all')
    fig6, ax = plt.subplots(len(channels),sharex=True, sharey=False)
    fig6.suptitle("W7-X ABES beam parameters, shot#"+shot)
    for i in range(len(channels)):
        d=read_tdms_data(channels[i],shot=shot,group_name=group_name,search_dir=search_dir,save_dir=save_dir)
