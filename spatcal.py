...
class ShotSpatCal(flap.DataObject):
    ...
    def full_calib(self, options={}):
        ...
        CalcCalibration(calibration_id, options=options)
    ...
    def read_fibre_calib_list(self, options):
        ...
        with open(filename) as fibre_list:
            ...
            for line_index in range(1, len(lines)):
                line_data = [data for data in lines[line_index].split("\t") if data != ""]
                self.fibre_calib_list += [[line_data[0], float(line_data[1]), float(line_data[2]), line_data[3], line_data[4][:-1]]]
    ...
    def process_apdcam_to_cmos_calib(self, options):
        ...
        for image_params in self.fibre_calib_list:
            ...
            if first_image == True:
                if int(image_params[4]) != int(self.calibration_id):
                    ...
                    a, B = self.calc_cmos_transform(options['Flip horizontally'])
                if options['Plot'] is False:
                    first_image = False

            image = plt.imread(os.path.join(options['Spatial calib source dir'], image_params[4], image_params[3]))
            ...
            try:
                ...
                x_center = np.average(np.arange(len(x_weight)), weights=x_weight)
                y_center = np.average(np.arange(len(y_weight)), weights=y_weight)
    
            except ZeroDivisionError: # this my happen for different calibration images, these need to be filtered differently
                image_filtered = image/np.var(image)
                image_filtered[np.where(image_filtered<np.max(image_filtered)*0.5)] = 0
        ...
        for image_params in self.fibre_calib_list:
            if image_params[0] in multi_chan_meas_id:
                chan_cent[image_params[0]] += [[image_params[1], image_params[2], image_params[5], image_params[6]]]
            else:
                chan_cent[image_params[0]] = [[image_params[1], image_params[2], image_params[5], image_params[6]]]
...
#            point_XY[index, 1] = np.dot(perp_to_optax, self.calibconf.image_y_vector)
    ...
    def calc_cmos_transform(self, flip_horizontally):
        ...
        if int(self.calibration_id) == 2021:
            ...
            fileID = [29, 56, 55, 4, 5, 6] # these correspond to the channels in the "channels" variable
            ...
            for fileindex in range(len(channels)):
                image = np.asarray(plt.imread(os.path.join(dir2018, rel_files2018[fileindex]))).astype("float")
                ...
                image = np.asarray(plt.imread(os.path.join(dir2021, rel_files2021[fileindex])))
                if flip_horizontally == True:
                    image = np.fliplr(image)
            ...
            a0 = np.mean([opt_chan_2021[channel]-opt_chan_2018[channel] for channel in channels], axis = 0)
            b0 = 0
            ...
            raise NotImplementedError("Currently only works for 2021 calibration")
