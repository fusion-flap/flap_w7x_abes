...
class ShotSpatCalCXRS(ShotSpatCal):
    ...
    def read(self, options={}):
        ...
        spatcal_dir = options['Shot spatcal dir']
        fn = os.path.join(spatcal_dir, self.shotID + '_cxrs_spat.cal')
        try:
            h5File = flap.load(fn)
            channel_names = np.array(h5File['Channels'])
            fibres = h5File["Fibres"]
        ...
        if 'Channels' in options.keys():
            ...
            channels = [bytes('A.'+str(index).zfill(2), 'utf-8')
        ...
        for ch in channels:
            if type(ch) == bytes:
                ch = ch.decode('utf-8')
            if len(np.where(channel_names == ch)[0]) > 0:
                relative_loc[np.where(channel_names == ch)[0][0]] = index+1
                index = index+1
        relative_loc = [x-1 if x >
                        0 else len(relative_loc) for x in relative_loc]
        relative_loc = np.argsort(relative_loc)
